@extends('backend.layouts.app', ['module' => 'Blogs', 'title' => 'Edit blog category'])
@section('content')
<form class="form-horizontal" method="POST" action="{{route('blogs.categories.update', $blogCategory->blog_category_id)}}">
    @csrf
    <div class="row">
        ...
        <div class="mb-3 col-md-6">
            ...
            <input type="text" class="form-control form-control-sm" id="name" name="name" value="{{$blogCategory->blog_category_name}}">
        </div>
        <div class="mb-3 col-md-6">
            <label for="slug" class="form-label">Blog category slug</label>
            <input type="text" class="form-control form-control-sm" id="slug" name="slug" value="{{$blogCategory->blog_category_slug}}">
        </div>
        <div class="mb-3 col-md-12">
            <label for="blog_category_content" class="form-label">Blog category content</label>
            <textarea class="form-control form-control-sm text-editor" id="blog_category_content" name="blog_category_content">{{$blogCategory->blog_category_description}}</textarea>
        </div>
        <div class="mb-3 col-md-12">
            ...
            <input type="text" class="form-control form-control-sm" id="seo_title" name="seo_title"  value="{{$blogCategory->seo_title}}">
        </div>
        <div class="mb-3 col-md-12">
            ...
            <textarea class="form-control form-control-sm" id="seo_description" name="seo_description">{{$blogCategory->seo_description}}</textarea>
        </div>
        <div class="mb-3 col-md-12">
            <label for="seo_keywords" class="form-label">SEO Keywords - <b  class="text-sm">Count : <span id="keywordCount">0</span></b></label>
            <textarea class="form-control form-control-sm" id="seo_keywords" name="seo_keywords">{{$blogCategory->seo_keywords}}</textarea>
        </div>
